...
void ServerApplication::recieve(QByteArray msg)
{
    ...
    case PRINT_CLASSIC_REQUEST:
        ...
        if (mode == COMPLEX_MODE){
            out = polinom->polinomWithRoots().str();
        } else if (mode == REAL_MODE){
            out = floatPolinom->polinomWithRoots().str();
        }
    ...
    case PRINT_CANONIC_REQUEST:
        answer << QString().setNum(PRINT_POLYNOM_ANSWER);
        if (mode == COMPLEX_MODE){
            out = polinom->polinomWithDegrees().str();
        } else if (mode == REAL_MODE){
            out = floatPolinom->polinomWithDegrees().str();
        }
        answer << QString::fromStdString(out);
        break;
    ...
    case ADD_ROOT_REQUEST:
        if (mode == COMPLEX_MODE) {
            ...
            polinom = Polinom<TComplex>().fill(An, roots, rootsAmount + 1);
        } else if (mode == REAL_MODE) {
            pos = msg.indexOf(separator.toLatin1());
            ...
            floatPolinom = Polinom<float>().fill(floatAn, floatRoots, rootsAmount + 1);
        }
        break;
    ...
    case VALUE_AT_POINT_REQUEST:
        if (mode == COMPLEX_MODE) {
            ...
            if ( rootsAmount == 0 ){
                ...
                TComplex value = polinom->valueInPoint(ComplexPoint);
            ...
            if ( rootsAmount == 0 ){
                ...
                float value = floatPolinom->valueInPoint(floatPoint);
