...
void ServerApplication::recieve(QByteArray msg)
{
    ...
    int index;
    int position = msg.indexOf(separator.toLatin1());
    int t = msg.left(position).toInt();
    msg.remove(0, 2);
    ...
    case CHANGE_ROOT_REQUEST:
        position = msg.indexOf(separator.toLatin1());
        index = msg.left(position).toInt();
        msg.remove(0, position + 1);
        msg >> complex;
        roots[index] = complex;
        polinom = Polinom().fill(An, roots, rootsAmount+1);
        break;
